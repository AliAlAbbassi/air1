...
from air1.agents.research.models import ICPProfile, ProspectInput

# Sample test data
SAMPLE_PROSPECT = ProspectInput(
    linkedin_username="alialabbassi",
    full_name="Ali Al Abbassi",
    headline="Senior Software Engineer at Anghami, Distributed Systems, ETL Platforms, Low Level Systems, GPUs",
    company_name="Anghami",
...
SAMPLE_ICP = ICPProfile(
    ...
    pain_points_we_solve=[
        "Lead Generation for Software Agencies",
        "LinkedIn AI Automation",
        "AI Agents Software Agency",
        "Vibecoding Agency",
    ],
    value_proposition="AI B2B SAAS for agencies and B2B Sellers",
    product_description="AI B2B SAAS for LinkedIn Automation",
)
...
class TestFullResearchCrewOnline:
    ...
    def test_full_research_crew(self, caplog):
        ...
        if result.ai_summary:
            logger.info("\nAI SUMMARY:")
            logger.info(
                f"Prospect Summary: {result.ai_summary.prospect_summary[:200]}..."
            )
            logger.info(
                f"Company Summary: {result.ai_summary.company_summary[:200]}..."
            )
            logger.info(
                f"Achievements: {result.ai_summary.notable_achievements_current_role}"
            )
            logger.info(f"Talking Points: {result.ai_summary.key_talking_points}")
            logger.info(
                f"Recommended Approach: {result.ai_summary.recommended_approach}"
            )
