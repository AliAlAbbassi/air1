...
SELECT sfd.sec_form_d_id AS "secFormDId",
       ...
       sfd.filing_date AS "filingDate",
       sfd.cik,
       so.first_name AS "officerFirstName",
       so.last_name AS "officerLastName",
       so.title AS "officerTitle"
FROM (
    SELECT fd.sec_form_d_id, fd.issuer_name, fd.total_offering_amount,
           fd.industry_group_type, fd.revenue_range, fd.minimum_investment,
           fd.total_investors, fd.is_equity, fd.is_new_offering,
           sf.filing_date, sf.cik
    FROM sec_form_d fd
    JOIN sec_filing sf ON sf.sec_filing_id = fd.sec_filing_id
